...
<template>
    <div class="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 w-full max-w-2xl p-8">
            <h1 class="text-xl font-semibold text-gray-900 text-center mb-6">Create Lesson</h1>
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div>
                    ...
                    <input
                        ...
                        v-model="form.name"
                        type="text"
                        class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="3"
                        class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none"
                ...
                <div>
                    ...
                    <div v-if="form.exercises.length === 0" class="text-xs text-gray-400 text-center py-4 border border-dashed border-gray-200 rounded-lg">
                        No exercises yet
                    </div>
                    ...
                    <ul v-else class="space-y-2">
                        <li
                            ...
                            :key="i"
                            class="flex items-center justify-between rounded-lg border border-gray-100 px-4 py-3 bg-gray-50"
                        >
                            <div>
                                <p class="text-sm font-medium text-gray-800">{{ ex.name }}</p>
                                <p class="text-xs text-gray-400">{{ EXERCISE_TYPES.find(t => t.value === ex.decision_type)?.label }}</p>
                            </div>
                            <div class="flex gap-3">
                                <button type="button" @click="openEdit(i)" class="text-xs text-indigo-500 hover:text-indigo-700">Edit</button>
            ...
            <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 space-y-4">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label class="text-sm font-medium text-gray-700">Clause</label>
                        <button type="button" @click="addClauseRow" class="text-xs text-indigo-600 hover:text-indigo-800">+ Add field</button>
                    </div>
                    <div class="space-y-2">
                        <div v-for="(row, i) in clauseRows" :key="i" class="flex gap-2 items-center">
                            <input
                                ...
                                type="text"
                                placeholder="key"
                                class="w-2/5 rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                            />
                            <input
                                v-model="row.value"
                                type="text"
                                placeholder="value"
                                class="flex-1 rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                            />
                            <button
                                type="button"
